...
class PrototypicalNetwork(Module):
    ...
    def distance_logits(a, b):
        n = a.shape[0]
        m = b.shape[0]
        logits = -((a.unsqueeze(1).expand(n, m, -1) - b.unsqueeze(0).expand(n, m, -1))**2).sum(dim=2)
        return logits

    def __init__(self, encoder, classifier, distance=distance_logits):
    ...
    def _inner_iter(self, x, y, params, mom_buffer, episode, inner_args, detach):
        ...
        with torch.enable_grad():
            ...
            grads = autograd.grad(loss, params.values(),
                                  create_graph=(
                                      not detach and not inner_args['first_order']),
    ...
    def _adapt(self, x, y, params, episode, inner_args, meta_train):
        ...
        for step in range(inner_args['n_step']):
            if self.efficient:  # checkpointing
                ...
                params = OrderedDict(
                    zip(params_keys, state[:len(params_keys)]))
                mom_buffer = OrderedDict(
                    zip(mom_buffer_keys, state[-len(mom_buffer_keys):]))
            else:
                params, mom_buffer = self._inner_iter(
