...
class MiniImageNetLoader(Dataset):
    ...
    def __init__(self, data_dir, split='train', image_size=84, normalization=True, transform=None):
        ...
        split_file = os.path.join(data_dir, split_tag + '.pickle')
        ...
        if normalization:
            ...
            self.norm_params = {'mean': [0., 0., 0.],'std':  [1., 1., 1.]}
...
class MetaMiniImageNetLoader(MiniImageNetLoader):
    # Meta collate function for Pytorch constructing DataLoader
    ...
    def __init__(self, data_dir, split='train', image_size=84, normalization=True, transform=None, val_transform=None, n_batch=200, n_episode=4, n_way=5, n_shot=1, n_query=15, meta_collate_fn = _meta_collate_fn):
        super(MetaMiniImageNetLoader, self).__init__(data_dir, split, image_size, normalization, transform)
        self.n_batch = n_batch
        self.n_episode = n_episode
        self.n_way = n_way
        self.n_shot = n_shot
        self.n_query = n_query
        self.meta_collate_fn = meta_collate_fn
